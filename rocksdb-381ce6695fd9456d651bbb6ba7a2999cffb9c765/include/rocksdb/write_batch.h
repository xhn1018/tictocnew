...
class WriteBatch : public WriteBatchBase {
 public:
  ...
  class Handler {
   ...
   virtual Status PutCF1(uint32_t column_family_id, const Slice& key,
                         const Slice& value, SequenceNumber shift){
         UNUSED(shift);
  ...
  void k(){
  std::cout<<rep_.size()<<std::endl;
  for(int i=0;i<(int)rep_.size();i++)
  {
      std::cout<<rep_.at(i)<<std::endl;
  }
 ...
 private:
  ...
  // remove duplicate keys. Re)move it when the hack is replaced with a proper
  // solution.
