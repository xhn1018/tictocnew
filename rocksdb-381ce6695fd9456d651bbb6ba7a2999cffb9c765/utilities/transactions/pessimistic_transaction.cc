  ...
  DBImpl* db_impl = static_cast_with_check<DBImpl, DB>(db_->GetRootDB());
   ...
   for (auto& key_map_iter : GetTrackedKeys()) { 
    ...
    for (const auto& key_iter1 : keys) {  
      //std::cout<<key_iter1.second.seq<<std::endl;
      //std::cout<<key_iter1.first<<std::endl;
      SequenceNumber w_ts ;
      SequenceNumber r_ts ;
      UnPackSequence(&w_ts,&r_ts, key_iter1.second.seq);
      //std::cout<<"3"<<std::endl;
      if(key_iter1.second.exclusive==1){
    ...
    GetWriteBatch()->GetWriteBatch()->TictocSequence =ttseq<<12;
    //GetWriteBatch()->GetWriteBatch()->k();
  s = db_impl->WriteWithCallback(
      write_options_, , &callback);
  return s;
}
...
Status PessimisticTransaction::TryLock(ColumnFamilyHandle* column_family,
    ...
    if (snapshot_) {
     ...
     // auto cfd = cfh->cfd();
      SuperVersion* sv = dbimpl_->GetAndRefSuperVersion(cfh_id);
       if (sv == nullptr) {
          result = Status::InvalidArgument("Could not access column family ");
           //std::cout<<"ok1"<<std::endl; 
       }
       Status s = dbimpl_->GetLatestSequenceForKey(sv, key_str, 0,
                                                &seq, &found_record_for_key);
       if (!(s.ok() || s.IsNotFound() || s.IsMergeInProgress())){

       
       }
       if(found_record_for_key == 0) {
           seq =0x1<<12; 
       }
       //std::cout<<seq<<std::endl;
        //std::cout<<found_record_for_key<<std::endl;  
       if (result.ok()) {
	 dbimpl_->ReturnAndCleanupSuperVersion(cfh_id, sv);
...
Status PessimisticTransaction::CheckTransactionForConflicts1(DB* db) {
    Status result;
...
Status PessimisticTransaction::CheckTransactionForConflicts(DB* db) {
    ...
    GetWriteBatch()->GetWriteBatch()->TictocSequence =1;
    //std::cout<<"ok"<<std::endl;
    //SetSequence(commit_time_batch_,ttseq);
    //int k=ttseq;
    //printf("%d",k);
    //std::cout<<ttseq<<std::endl;
