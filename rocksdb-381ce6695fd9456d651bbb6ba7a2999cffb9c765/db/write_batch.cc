...
bool readtimestamp(Slice *input){
    SequenceNumber s;
    std::string k;
    for(int i=0;i<3;i++){
        k.push_back((*input)[i]);
...
SequenceNumber Getshift(Slice *input){
    SequenceNumber s;
    std::string k;
    for(int i=0;i<3;i++){
        k.push_back((*input)[i]);
...
Status ReadRecordFromWriteBatch(Slice* input, char* tag,
  ...
  *column_family = 0;  // default
  switch (*tag) {
    ...
    case kTypeValue:
      if (!GetLengthPrefixedSlice(input, key) ||readtimestamp(input)||
  ...
  while (((s.ok() && !input.empty()) || UNLIKELY(s.IsTryAgain())) &&
    ...
    switch (tag) {
      ...
      case kTypeValue:
        ...
        s = handler->PutCF1(column_family, key, value,shift);
        if (LIKELY(s.ok())) {
          empty_batch = false;
          found++;
        }
...
class MemTableInserter : public WriteBatch::Handler {
 ...
 public:
  ...
  Status PutCFImpl1(uint32_t column_family_id, const Slice& key,
                   const Slice& value, ValueType value_type, SequenceNumber shift1) {
    ...
    if (!moptions->inplace_update_support) {
      bool mem_res =
          mem->Add(sequence_+shift1, value_type, key, value,
      ...
      if (mem->UpdateCallback(sequence_, key, value)) {
        ...
        if (status == UpdateStatus::UPDATED_INPLACE) {
          ...
          mem_res = mem->Add(
              sequence_+shift1, value_type, key, Slice(prev_buffer, prev_size));
          assert(mem_res);
          ...
          bool mem_res __attribute__((__unused__));
          mem_res =
              mem->Add(sequence_+shift1, value_type, key, Slice(merged_value));
          assert(mem_res);
          RecordTick(moptions->statistics, NUMBER_KEYS_WRITTEN);
        }
      }
 ...
 Status PutCF(uint32_t column_family_id, const Slice& key,
  ...
  Status PutCF1(uint32_t column_family_id, const Slice& key,
                       const Slice& value, SequenceNumber shift1) override {
    return PutCFImpl1(column_family_id, key, value, kTypeValue, shift1);
  }
